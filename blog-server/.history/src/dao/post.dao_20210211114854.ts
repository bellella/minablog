...
class PostDAO {
    public getBasePostQuery() {
        let postQuery: any = {
            attributes: ['id', 'title', 'createdAt', 'content', 'thumbnail', 'view', 'show'
    ...
    public recommendList(postId, tagList = [], categoryId) {
        let query: any = {
            attributes: ['id', 'title', 'createdAt', 'content', 'thumbnail', 'view', 'categoryId'],
            where: { show: 1, categoryId, id: { [sequelize.Op.ne]: postId } },
        ...
        if (tagList.length != 0) {
            query.attributes.push([sequelize.fn('COUNT', sequelize.col("tags.post_id")), 'tagCount']);
            query['include'] = [
                { attributes: [], model: PostTagModel.getModel(), required: false, as: 'tags', duplicating: false, where: { name: tagList } } // , where: { name : tagList }
            ];
    ...
    public searchList(type: string, value: string) {
        ...
        if (type == 'tag') {
            ...
            if (value && value !== 'null') {
                query['where'] = { title: { [sequelize.Op.like]: `%${value}%` } };
            }
        }
        return PostModel.getModel().findAll(query);
    }
