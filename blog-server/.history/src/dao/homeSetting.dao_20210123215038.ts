...
class HomeSettingDAO {
    ...
    public list(): Promise<HomeSettingAttribute[]> {
        return HomeSettingModel.getModel().findAll({
            order: [['priority', 'asc']]
        });
    }

    public listBlog(): Promise<HomeSettingAttribute[]> {
        return HomeSettingModel.getModel().findAll({
            where: { show: 1 },
            order: [['priority', 'asc']]
        });
    ...
    public async delete(settingId: string) {
        return HomeSettingModel.getModel().destroy({ where: { id: settingId } });
    }
}
