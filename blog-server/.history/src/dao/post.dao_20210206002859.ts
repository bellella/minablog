...
class PostDAO {
    ...
    public recommendList(tagList = [], categoryId) {
        ...
        if(tagList.length != 0) {
            query['attributes'] = ['id', 'title', 'createdAt', 'content', 'thumbnail', 'view', 'categoryId', [sequelize.fn('COUNT', sequelize.col('tags')), 'tagCount']];
        
            query['include'] = [
                {model: PostTagModel.getModel(), required: false, as: 'tags', where: { name : tagList }}
            ],
